...
export class FilterDropdownComponent {
  ...
  checkFilter(Id:number,event:any)
  {
    ...
    {
      this.SelectedCount-=1;
      this.selectedFilterIds=this.selectedFilterIds.filter(item=>item!=Id);
    }
  ...
  reset()
  {
    const checkboxes = document.querySelectorAll('.status-check') as NodeListOf<HTMLInputElement>;
